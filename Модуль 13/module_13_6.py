...
[kb_start.add(button) for button in buttons]

kb_main = InlineKeyboardMarkup(resize_keyboard=True)
buttons = [InlineKeyboardButton(text=text, callback_data=call)
           for text, call in (('Рассчитать норму калорий', 'calories'), ('Формулы расчёта', 'formulas'))]
[kb_main.add(button) for button in buttons]
