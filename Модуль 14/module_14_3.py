...
# list_products = ['Росточки здоровья №14', 'Росточки здоровья №17',
#                  'Сильный иммунитет', 'Мужское здоровье']
list_products = ['Продукт 1', 'Продукт 2',
                 'Продукт 3', 'Продукт 4']
list_description = ['Каша «Биозолото»', 'Каша «Бодрость»',
                    'Для укрепления иммунитета', 'От природы-матушки с любовью!']
kb_products = InlineKeyboardMarkup(
    inline_keyboard=[[InlineKeyboardButton(text=text, callback_data='product_buying')
                      for text in list_products
                      ]], resize_keyboard=True)
...
async def get_buying_list(message):
    for i, e in enumerate(zip(list_products, list_description), start=1):
        with open(f'images/{i}.jpg', 'rb') as img:
            await message.answer_photo(img, f'Название: {e[0]} | Описание: {e[1]} | Цена: {i * 100}')
